...
Page({
  ...
  async PopSelect() {
    ...
    res.data.forEach(item => {
      columns.push({text:item.name,id:item.id})
    })
    this.setData({
      columns,
      show: true
  ...
  async confirm() {
    ...
    publishData.itemCategory = category_id
    publishData.place = place
    publishData.detail = detail
    publishData.contact = phone
    publishData.useropenid = app.data.openid
    publishData.imguuid = uuid.data.message
    const p = await publish(publishData)
    console.log(p.data,"我发送的数据")
    if(p.data>0){
      wx.showToast({
        title: '发布成功',
        icon:"success"
